...
export default function Auth() {
    ...
    useEffect(() => {
        if (number) {
            ...
            setNumber(number.split("").filter(n => numbers.includes(n)).join(""));
        }
    }, [number]);
    ...
    return (
        <div className='w-full h-screen relative'>
            <img src={role === "student" ? student : teacher} className='w-full h-full object-cover' alt={role === "student" ? "Student image" : "Teacher image"} />
                ...
                <div className="box w-80 h-100 flex flex-col items-center">
                    ...
                    {isLogin ? (
                        <form action="" className='flex flex-col mt-5 gap-5'>
                            ...
                            <button type="submit" className="login text-white text-2xl bg-cyan-400 py-3 rounded-full">Kirish</button>

                            <span className="text-white text-center">Yangimisiz? <span className="text-cyan-300 cursor-pointer transition-all duration-500 hover:text-cyan-500" onClick={() => setisLogin(!isLogin)}>Ro'yxatdan o'ting.</span></span>
                        </form>
                    ) : (
                        <form action="" className='flex flex-col mt-5 gap-5'>
                            ...
                            <div className="flex items-center">
                                ...
                                <input
                                    ...
                                    value={number || ""}
                                    type="text"
                                    className='w-full text-white rounded-lg px-5 text-xl py-2 min-w-80 outline-none border-2 border-cyan-300 rounded-tl-none rounded-bl-none '
